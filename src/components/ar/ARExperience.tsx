  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-background text-foreground">
      ...
      <header className="absolute inset-x-0 top-0 z-20 flex items-center justify-between gap-4 p-5">
        <div>
          <h1 className="font-mono text-sm tracking-[0.35em] text-[rgb(34,255,225)] uppercase">Aurora // Handspace</h1>
          <p className="mt-1 font-mono text-[11px] tracking-widest text-white/60 uppercase">gesture interface v2.6</p>
        </div>
        <div className="flex items-center gap-2">
          <StatusPill label={status === "ready" ? (handPresent ? "hand locked" : "scanning") : status} on={status === "ready" && handPresent} />
          <button onClick={() => { sfx.hover(); setHud((v) => !v); }} className="rounded-full border border-[rgba(34,255,225,0.4)] bg-black/40 px-4 py-2 font-mono text-[11px] tracking-widest text-[rgb(34,255,225)] uppercase backdrop-blur transition hover:bg-[rgba(34,255,225,0.15)]">HUD {hud ? "on" : "off"}</button>
        </div>
      </header>
      ...
      <div className={`absolute inset-x-0 top-24 z-20 mx-auto w-[min(92vw,720px)] transition-all duration-500 ${menuOpen ? "translate-y-0 scale-100 opacity-100 blur-0" : "pointer-events-none -translate-y-10 scale-95 opacity-0 blur-md"}`}>
        <div className="relative rounded-3xl border border-[rgba(34,255,225,0.45)] bg-[rgba(4,12,22,0.55)] p-6 backdrop-blur-xl shadow-[0_0_80px_rgba(34,255,225,0.25)]">
          ...
          <p className="mt-5 font-mono text-[11px] tracking-widest text-white/45 uppercase">{active ? `module engaged · ${active}` : "awaiting selection"}</p>
        </div>
        {menuOpen && <p className="mt-4 flex items-center justify-center gap-2 font-mono text-[11px] tracking-[0.3em] text-[rgb(34,255,225)]/80 uppercase"><Crosshair className="h-3.5 w-3.5" />point with index finger · pinch thumb + index to click</p>}
      </div>

      {menuOpen && pointer.active && status === "ready" && (
        <div className="pointer-events-none fixed z-40 -translate-x-1/2 -translate-y-1/2" style={{ left: pointer.x, top: pointer.y }}>
          <svg width="76" height="76" viewBox="0 0 76 76" className="overflow-visible">
            <circle cx="38" cy="38" r="22" fill="none" stroke={hovered ? "rgba(255,90,210,0.5)" : "rgba(34,255,225,0.45)"} strokeWidth="2" />
            <circle cx="38" cy="38" r="22" fill="none" stroke={hovered ? "rgb(255,90,210)" : "rgb(34,255,225)"} strokeWidth="4" strokeLinecap="round" strokeDasharray={2 * Math.PI * 22} strokeDashoffset={2 * Math.PI * 22} transform="rotate(-90 38 38)" style={{ filter: "drop-shadow(0 0 8px rgba(34,255,225,0.9))" }} />
            <circle cx="38" cy="38" r="4" fill="rgb(34,255,225)" />
            <path d="M38 6 V18 M38 58 V70 M6 38 H18 M58 38 H70" stroke="rgba(34,255,225,0.8)" strokeWidth="2" strokeLinecap="round" />
          </svg>
        </div>
      )}
    </div>
...
function StatusPill({ label, on }: { label: string; on: boolean }) {
  return <span className="flex items-center gap-2 rounded-full border border-white/15 bg-black/40 px-3 py-2 font-mono text-[11px] tracking-widest text-white/75 uppercase backdrop-blur"><span className={`h-2 w-2 rounded-full ${on ? "bg-[rgb(34,255,225)] shadow-[0_0_10px_rgba(34,255,225,0.8)]" : "bg-white/30"}`} />{label}</span>;
}
